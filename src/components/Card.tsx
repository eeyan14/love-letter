...
const Card = ({ character, size }: CardProps) => {
  const descriptions: {
    [key: string]: {
      reference: string,
      card: string,
  ...
  const renderCharacterReference = () => {
    ...
    return Object.keys(descriptions).map((key, i) => {
      return (
        <p><span className="medieval">{highestNumber - i}–{titleize(key)}</span> <span className="small">(x1):</span> {descriptions[key].reference}</p>
      )
    })
  }
  ...
  return (
    <div className={`card size-${size} character-${character}`}>
      ...
      {characters.includes(character) && (
        <div>
          <p><span className="medieval">{titleize(character)}</span></p>
